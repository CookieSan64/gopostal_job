...
--------------------------------------------------------------------------------
-- NE RIEN MODIFIER
local CurrentDelivery 		  = false -- Le joueur en livraison ?
...
-- 									VEHICLE	
function GenerateRandomPlate(length)
    local plateCharset = '0123456789'
    local plate = 'GOPOSTAL'
    math.randomseed(GetGameTimer())

    for i = 1, length - string.len(plate) do
        local randomChar = string.sub(plateCharset, math.random(1, #plateCharset), math.random(1, #plateCharset))
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                ...
                ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Heading, function(vehicle)
                    ...
                    local randomPlate = GenerateRandomPlate(9) -- "GOPOSTAL" + 1 chiffre aléatoire
                    SetVehicleNumberPlateText(vehicle, randomPlate) -- Affecter la plaque d'immatriculation aléatoire
